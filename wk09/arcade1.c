...
int main(int argc, char* argv[]) {
    ...
    while (inuse) {
        ...
        switch (input) {
            case TAMAGOTCHI:
                printf("hello from case TAMAGOTCHI!\n");
                strcpy(mygame, "./tamagotchi\0"); 
                break;
            case COWBOY:
                printf("hello from case COWBOY!\n");
        ...
        /** If mygame is not an empty string, fork the current process and swap
         * the child process image using execvp to run the chosen game. */
        if (strlen(mygame) != 0) {
            pid = fork();
            if (pid == 0) {
